...
class BlogImageType extends ObjectModel
{
    ...
    {
        @trigger_error(
            'self::stripslashes() is deprecated since version 8.0.0. Use PHP\'s stripslashes instead.',
            E_USER_DEPRECATED
        );
    ...
    public static function ImageGenerate()
    {
        ...
        foreach ($category_types as $image_type) {
            foreach ($get_cate_image as $cat_img) {
                ...
                ImageManager::resize($path, _PS_MODULE_DIR_ . 'smartblog/images/category/' . $cat_img['id_smart_blog_category'] . '-' . self::stripslashes($image_type['type_name']) . '.jpg', (int) $image_type['width'], (int) $image_type['height']
                );
            }
        }
        foreach ($posts_types as $image_type) {
            foreach ($get_blog_image as $blog_img) {
                ...
                ImageManager::resize($path, _PS_MODULE_DIR_ . 'smartblog/images/' . $blog_img['id_smart_blog_post'] . '-' . self::stripslashes($image_type['type_name']) . '.jpg', (int) $image_type['width'], (int) $image_type['height']
                );
            }
        }
        foreach ($author_types as $author_type) {
            ImageManager::resize($get_author_image, _PS_MODULE_DIR_ . 'smartblog/images/avatar/avatar-' . self::stripslashes($author_type['type_name']) . '.jpg', (int) $author_type['width'], (int) $author_type['height']
            );
        }
    }
    ...
    public static function ImageDelete()
    {
        ...
        foreach ($category_types as $image_type) {
            foreach ($get_cate_image as $cat_img) {
                ...
                $dir = _PS_MODULE_DIR_ . 'smartblog/images/category/' . $cat_img['id_smart_blog_category'] . '-' . self::stripslashes($image_type['type_name']) . '.jpg';
                if (file_exists($dir))
                    unlink($dir);
            }
        }
        foreach ($posts_types as $image_type) {
            foreach ($get_blog_image as $blog_img) {
                ...
                $dir = _PS_MODULE_DIR_ . 'smartblog/images/' . $blog_img['id_smart_blog_post'] . '-' . self::stripslashes($image_type['type_name']) . '.jpg';
                if (file_exists($dir))
                    unlink($dir);
            }
        }
        foreach ($author_types as $image_type) {
            $dir = _PS_MODULE_DIR_ . 'smartblog/images/avatar/avatar-' . self::stripslashes($image_type['type_name']) . '.jpg';
            if (file_exists($dir))
                unlink($dir);
        }
    }
