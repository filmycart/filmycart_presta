$(document).ready(function(){
    ...
    function init() {
        ...
        $(document).on('click', '#posnewsletterpopup .pnp-close, #posnewsletterpopup-overlay', function () {
             // Thêm zoomOutBox cho content
    var $content = $pnp.find('.posnewsletter-content');
    $content.removeClass('zoomInbox').addClass('zoomOutBox');

    if ($("#pnp-checkbox").is(':checked')) {
        setCookie(0);
    }

    // Chờ animation xong mới ẩn popup + overlay
    $content.one('animationend', function () {
        $pnp.removeClass('showed-pnp');
        $overlay.removeClass('showed-popup'); // tắt nền sau cùng
        $content.removeClass('zoomOutBox');   // reset
    });
        });
